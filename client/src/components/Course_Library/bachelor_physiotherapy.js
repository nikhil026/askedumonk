import React, { Component } from "react";
import { Link } from "react-router-dom";
class BOPhysiotherapy extends Component {
  render() {
    return (
      <div className="wrapper">
        <section className="coursedetauilstop">
          <div className="container1">
            <div className="row">
              <div className="col-md-2 col-xs-2">
                <img
                  alt="Agriculture course"
                  src={require("../.././images/bachelor-of-physiotherapy.medical.jpg")}
                />
              </div>
              <div className="col-md-6 col-xs-10 clgprofiledes">
                <h3>Bachelor of Physiotherapy</h3>
                <h6 className="locationclg">
                  <i className="fa fa-graduation-cap" />
                  Medical &amp; Health Car
                </h6>
                <h6 className="locationclg">
                  <i className="fa fa-book" />
                  Degree Type : 12,
                </h6>
              </div>
              <div className="col-md-4 col-xs-12 mt-15 blockbtn">
                <Link to="/medical" className="crssidebtn">
                  All Medical &amp; Health Care Courses
                </Link>
              </div>
            </div>
          </div>
        </section>
        <div className="content-wrapper coursebodypa">
          <div className="container">
            <div className="col-md-12 crstextstyle crsdetailscnt" role="main">
              <div className=" detail-content-wrapper">
                <div id="section-5" className="detail-content coursedessside">
                  <h2 className="heading-new">
                    <span>Overview</span>
                  </h2>
                  <div className="coursecontdes">
                    <p>
                      <strong />
                    </p>
                    <p>
                      <span style={{ "font-size": "14px" }}>
                        <strong>Bachelor of Physiotherapy (BPT)</strong>
                      </span>
                      <span style={{ "font-size": "14px" }}>
                        <strong>&nbsp;</strong>is a <strong>4-year</strong>{" "}
                        college course, including obligatory i.e. a half year of
                        the clinical temporary job, and including the
                        investigation of physical development went for averting
                        malady and incapacity. <strong>BPT</strong> is an
                        undergraduate course in <strong>Physiotherapy</strong>
                        .&nbsp;
                      </span>
                    </p>
                    <p>
                      <span style={{ "font-size": "14px" }}>
                        Physiotherapy uses
                        <strong>&nbsp;exercise, massage</strong> and other
                        methods for providing treatment to the patient whose
                        body movements are threatened by{" "}
                        <strong>aging, disease, injury</strong> or other
                        factors.
                      </span>
                    </p>
                    <p>
                      <span style={{ "font-size": "14px" }}>
                        The field of physiotherapy is likewise known by the name
                        of<strong>&nbsp;Exercise-based Recuperation.</strong>{" "}
                        Physiotherapy is a piece of the&nbsp;
                        <strong>Rehabilitative solution</strong>. It centers
                        around the utilization of{" "}
                        <strong>
                          non-intrusive treatment, rubs, physical developments
                          and exercise to enhance and cure wounds,
                          disfigurements, and sicknesses.&nbsp;
                        </strong>
                        Alongside restoration, some other critical segments of
                        physiotherapy are-
                        <strong>
                          conclusion, examination and evaluation of the
                          condition
                        </strong>
                        .
                      </span>
                    </p>
                  </div>
                </div>
                <div id="section-6" className="detail-content coursedessside">
                  <h2 className="heading-new">
                    <span>Courses &amp; Eligibility</span>
                  </h2>
                  <div className="coursecontdes">
                    <p>
                      <strong>
                        <span style={{ "font-size": "14px" }}>
                          For <strong>Bachelor of Physiotherapy (BPT)</strong>{" "}
                          course, a candidate must have the following
                          educational eligibilities:
                        </span>
                      </strong>
                    </p>
                    <ul>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>10+2 Pass&nbsp;</strong>in{" "}
                          <strong>Science Stream&nbsp;</strong>(i.e. Physics,
                          Chemistry and Biology as core subjects)
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          Must have passed<strong>&nbsp;</strong>the
                          <strong> Common Entrance Test (CET),&nbsp;</strong>
                          <span style={{ "font-size": "14px" }}>
                            <strong>
                              <Link to="http://careerpotli.com/exam-details/jipmer">
                                JIPMER
                              </Link>{" "}
                              etc.
                            </strong>
                          </span>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          Candidate should be at least
                          <strong>&nbsp;17 years</strong> of age
                        </span>
                      </li>
                    </ul>
                  </div>
                </div>
                <div id="section-7" className="detail-content coursedessside">
                  <h2 className="heading-new">
                    <span>Course Curriculum</span>
                  </h2>
                  <div className="coursecontdes">
                    <p>
                      <span style={{ "font-size": "14px" }}>
                        The syllabus of the course of <strong>BPT&nbsp;</strong>
                        <strong>(Bachelor of Physiotherapy)</strong> may differ
                        from college to college, but the structure is the same.
                        A definite syllabus of the same is as per the following:
                      </span>
                    </p>
                    <p>
                      <br />
                    </p>
                    <table style={{ width: "98%", marginright: "calc(2%)" }}>
                      <tbody>
                        <tr>
                          <td
                            style={{
                              width: "33.3333%",
                              textalign: "center",
                              verticalalign: "top",
                              backgroundColor: "rgb(239, 239, 239)"
                            }}
                          >
                            <span style={{ color: "rgb(85, 57, 130)" }}>
                              <strong>
                                <u>
                                  <span
                                    style={{
                                      "font-size": "18px",
                                      fontfamily: "Tahoma, Geneva, sans-serif"
                                    }}
                                  >
                                    Year I
                                  </span>
                                </u>
                              </strong>
                            </span>
                            <br />
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              <br />
                              >Anatomy
                              <br />
                              Physiology
                              <br />
                              Biochemistry
                              <br />
                              Biomechanics
                              <br />
                              Psychology
                              <br />
                              Sociology
                              <br />
                              Basic Nursing
                              <br />
                              Orientation to Physiotherapy
                              <br />
                              Integrated Seminars / PBL sessions
                            </span>
                            <br />
                            <br />
                          </td>
                          <td
                            style={{
                              width: "33.3333%",
                              verticalalign: "top",
                              textalign: "center"
                            }}
                          >
                            <span
                              style={{
                                color: "rgb(226, 8, 65)",
                                "font-size": "18px",
                                fontfamily: "Tahoma, Geneva, sans-serif"
                              }}
                            >
                              <strong>
                                <u>Year II</u>
                              </strong>
                            </span>
                            <br />

                            <span style={{ "font-size": "14px" }}>
                              Pathology
                              <br />
                              Microbiology
                              <br />
                              Pharmacology
                              <br />
                              Exercise Therapy
                              <br />
                              Electrotherapy
                              <br />
                              Research Methodology &amp; Biostatistics
                              <br />
                              First Aid &amp; CPR
                              <br />
                              Constitution of India
                              <br />
                              Introduction to Treatment
                              <br />
                              Clinical Observation Posting
                            </span>
                          </td>
                        </tr>
                      </tbody>
                    </table>{" "}
                    <table style={{ width: "98%", marginright: "calc(2%)" }}>
                      <tbody>
                        <tr>
                          <td
                            style={{
                              width: "33.3333%",
                              verticalalign: "top",
                              textalign: " center",
                              backgroundcolor: " rgb(239, 239, 239)"
                            }}
                          >
                            <span style={{ color: "rgb(41, 105, 176)" }}>
                              <strong>
                                <u>
                                  <span
                                    style={{
                                      fontfamily: "Tahoma, Geneva, sans-serif",
                                      fontsize: "18px"
                                    }}
                                  >
                                    Year III
                                  </span>
                                </u>
                              </strong>
                            </span>
                            <br />
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              General Medicine
                              <br />
                              General Surgery
                              <br />
                              Orthopedics &amp; Traumatology
                              <br />
                              Orthopedics and Sports Physiotherapy
                              <br />
                              Cardio-Respiratory &amp; General Physiotherapy
                              <br />
                              Allied Therapies
                              <br />
                              Supervised Rotatory Clinical Training - I
                            </span>
                          </td>
                          <td
                            style={{
                              width: "33.3333%",
                              verticalalign: "top",
                              textalign: "center"
                            }}
                          >
                            <span
                              style={{
                                color: "rgb(226, 8, 65)",
                                "font-size": "18px",
                                fontfamily: "Tahoma, Geneva, sans-serif"
                              }}
                            >
                              <strong>
                                <u>Year IV</u>
                              </strong>
                            </span>
                            <br />

                            <span style={{ "font-size": "14px" }}>
                              Neurology &amp; Neurosurgery
                              <br />
                              Neuro-Physiotherapy
                              <br />
                              Community Medicine
                              <br />
                              Community-Based Rehabilitation
                              <br />
                              Ethics, Administration and Supervision
                              <br />
                              Evidence-Based Physiotherapy Practice
                              <br />
                              Supervised Rotatory Clinical Training - II
                              <br />
                              Project
                            </span>
                          </td>
                        </tr>
                      </tbody>
                    </table>
                    <p>
                      <br />
                    </p>
                  </div>
                </div>
                <div id="section-8" className="detail-content coursedessside">
                  <h2 className="heading-new">
                    <span>Career Prospect</span>
                  </h2>
                  <div className="coursecontdes">
                    <p>
                      <span style={{ "font-size": "14px" }}>
                        <strong>BPT&nbsp;</strong>(
                        <strong>Bachelor of Physiotherapy</strong>) Graduate can
                        work as a:
                      </span>
                    </p>
                    <ul>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Research Assistant</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Lecturer</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Assistant Physiotherapist</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Researcher</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Therapy Manager</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Sports Physio Rehabilitator</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Osteopath</strong>
                        </span>
                      </li>
                    </ul>
                    <p>
                      <span style={{ "font-size": "14px" }}>
                        <strong>BPT&nbsp;</strong>(
                        <strong>Bachelor of Physiotherapy</strong>) Graduate
                        work areas are:
                      </span>
                    </p>
                    <ul>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>
                            Centres for physically and mentally challenged
                            people
                          </strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Community health centers</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Industries</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Government hospitals</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Mental health centers</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Nursing homes/ private care centers</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Private and public hospitals</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Private practice</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Rehabilitation centers</strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>Sports clinics and fitness centers</strong>
                        </span>
                      </li>
                      <li>
                        <strong>
                          <span style={{ "font-size": "14px" }}>
                            Universities and Colleges
                          </span>
                        </strong>
                      </li>
                    </ul>
                  </div>
                </div>
                <div id="section-9" className="detail-content coursedessside">
                  <h2 className="heading-new">
                    <span>Pay &amp; Remuneration</span>
                  </h2>
                  <div className="coursecontdes">
                    <p>
                      <span style={{ "font-size": "14px" }}>
                        Employees with a{" "}
                        <strong>Bachelor of Physiotherapy&nbsp;</strong>Degree,
                        the average minimum salary ranges by the position of job
                        and their experience and also the company and condition
                        of the company.
                      </span>
                    </p>
                    <p>
                      <span style={{ "font-size": "14px" }}>
                        Salary of a <strong>BPT</strong> graduate may be{" "}
                        <strong>Rs. 5,000 to Rs.30,000 per month</strong> for
                        fresher. It may increase with working experience and
                        eligibility of employee.
                      </span>
                    </p>
                  </div>
                </div>
                <div id="section-10" className="detail-content coursedessside">
                  <h2 className="heading-new">
                    <span>Famous Personalities</span>
                  </h2>
                  <div className="coursecontdes">
                    <p>
                      Some of the famous personalities in the field of{" "}
                      <strong>BPT (Bachelor of Physiotherapy)&nbsp;</strong>are
                      as follows:
                    </p>
                    <ul>
                      <li>
                        <strong>Eric Robertson (31 October 1965)</strong>
                      </li>
                      <li>
                        <strong>
                          <strong>Ben Fung</strong>
                        </strong>
                      </li>
                      <li>
                        <strong>Jerry Durham</strong>
                      </li>
                      <li>
                        <strong>Kyle Ridgeway</strong>
                      </li>
                      <li>
                        <strong>Adam Meakins</strong>
                      </li>
                      <li>
                        <strong>Diane Lee</strong>
                      </li>
                      <li>
                        <strong>Mike Reinold</strong>
                      </li>
                      <li>
                        <strong>Dr. Ali Irani</strong>
                      </li>
                      <li>
                        <strong>Hemakshi Basu</strong>
                      </li>
                      <li>
                        <strong>Dr. Sanjay Bakshi</strong>
                      </li>
                    </ul>
                  </div>
                </div>
                <div id="section-11" className="detail-content coursedessside">
                  {" "}
                  <h2 className="heading-new">
                    <span>Top Colleges in India</span>
                  </h2>
                  <div className="coursecontdes">
                    <p>
                      <span style={{ "font-size": "14px" }}>
                        There are various Government as well as Private colleges
                        available for the course of <strong>BPT</strong> across
                        India. Some of the famous and top colleges are as
                        follows:
                      </span>
                    </p>
                    <table style={{ width: "100%" }}>
                      <tbody>
                        <tr>
                          <td
                            colspan="2"
                            style={{
                              width: "99.248%",
                              verticalalign: " top",
                              textalign: "center"
                            }}
                          >
                            <span style={{ "font-size": " 18px" }}>
                              <span style={{ color: " rgb(85, 57, 130)" }}>
                                <strong>
                                  <u>
                                    <span
                                      style={{
                                        fontfamily:
                                          "                                    Tahoma, Geneva, sans-serif"
                                      }}
                                    >
                                      Top colleges for BPT
                                    </span>
                                  </u>
                                </strong>
                              </span>
                            </span>
                            <span style={{ "font-size": "14px" }}>
                              <br />
                            </span>
                          </td>
                        </tr>
                        <tr>
                          <td
                            style={{
                              width: "50%",
                              verticalalign: "top",
                              textalign: "center ",
                              backgroundcolor: "rgb(239, 239, 239)"
                            }}
                          >
                            <span style={{ "font-size": "14px" }}>
                              Apollo College of Physiotherapy,{" "}
                              <strong>Hyderabad</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              City College of Physiotherapy, &nbsp;
                              <strong>Mangalore</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              Indian Institute of Health Education &amp;
                              Research, <strong>Patna</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              JSS College of Physiotherapy,{" "}
                              <strong>Mysore</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              Institute of Medical Sciences Banaras Hindu
                              University, <strong>Varanasi</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              Institute of Physically Handicapped,{" "}
                              <strong>New Delhi</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              Kasturba Medical College, <strong>Manipal</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              S.D.M. College of Physiotherapy,{" "}
                              <strong>Dharwad</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              Post Graduate Institute of Medical Education and
                              Research, <strong>Chandigarh</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              Mahatma Gandhi University Medical College,{" "}
                              <strong>Kottayam</strong>
                            </span>
                            <br />
                            <div data-empty="true">
                              <span style={{ "font-size": "14px" }}>
                                Guru Gobind Indraprastha University,{" "}
                                <strong>New Delhi</strong>
                              </span>
                            </div>
                          </td>
                          <td
                            style={{
                              width: "50%",
                              verticalalign: "top",
                              "text-align": "center"
                            }}
                          >
                            <span style={{ "font-size": "14px" }}>
                              Manipal University (MU), <strong>Manipal</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              KEM Hospital, <strong>Mumbai</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              Lokmanya Tilak Municipal Medical College (LTMMC),{" "}
                              <strong>Mumbai</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              Shri Ayurveda Mahavidyalaya,{" "}
                              <strong>Nagpur</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              National Institute of Rehabilitation Training and
                              Research, <strong>Cuttak</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              K.J. Somaiya College of Physiotherapy &amp;
                              Research Centre, <strong>Mumbai</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              College of Physiotherapy,{" "}
                              <strong>Ahmednagar</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              Dayanand Ayurveda College,{" "}
                              <strong>Jalandhar</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              M.G.M. Medical College, <strong>Indore</strong>
                            </span>
                            <br />
                            <span style={{ "font-size": "14px" }}>
                              Institute of Post Graduate Medical Education and
                              Research, <strong>Kolkata</strong>
                            </span>
                            <br />

                            <br />
                            <br />
                            <div data-empty="true">
                              <br />
                            </div>
                          </td>
                        </tr>
                      </tbody>
                    </table>
                    <p>
                      <br />
                    </p>
                  </div>
                </div>{" "}
                <div id="section-15" class="detail-content coursedessside">
                  <h2 class="heading-new">
                    <span>More Information</span>
                  </h2>
                  <div class="coursecontdes">
                    <p>
                      <span style={{ "font-size": "14px" }}>
                        Benefits of{" "}
                        <strong>BPT (Bachelor of Physiotherapy)</strong> course:
                      </span>
                    </p>
                    <ul>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>
                            A candidate who has attained the BPT degree can set
                            up their own Physiotherapy clinic.
                          </strong>
                        </span>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>
                            The demand for good physiotherapists is rising day
                            by day.
                          </strong>
                        </span>
                      </li>
                    </ul>
                    <p>
                      <span style={{ "font-size": "14px" }}>
                        <br />{" "}
                      </span>
                    </p>
                    <p>
                      <span style={{ "font-size": "14px" }}>
                        Treatment is made to the following conditions through{" "}
                        <strong>Physiotherapy</strong>:
                      </span>
                    </p>
                    <ul>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>
                            Post accident treatment and rehabilitation
                          </strong>
                        </span>
                      </li>
                      <li>
                        <strong>
                          <span style={{ "font-size": "14px" }}>
                            Back and neck pain
                          </span>
                        </strong>
                      </li>
                      <li>
                        <strong>
                          <span style={{ "font-size": "14px" }}>
                            Cancer and palliative care
                          </span>
                        </strong>
                      </li>
                      <li>
                        <strong>
                          <span style={{ "font-size": "14px" }}>
                            Spinal and joint conditions such as arthritis
                          </span>
                        </strong>
                      </li>
                      <li>
                        <strong>
                          <span style={{ "font-size": "14px" }}>
                            Problems affecting children including cerebral palsy
                            and spina bifida
                          </span>
                        </strong>
                      </li>
                      <li>
                        <strong>
                          <span style={{ "font-size": "14px" }}>
                            Pregnancy-related problems such as back pain
                          </span>
                        </strong>
                      </li>
                      <li>
                        <strong>
                          <span style={{ "font-size": "14px" }}>
                            Work-related injuries
                          </span>
                        </strong>
                      </li>
                      <li>
                        <strong>
                          <span style={{ "font-size": "14px" }}>
                            Heart/lung conditions like asthma, obstructive
                            airways diseases (COPD) and other breathing
                            difficulties
                          </span>
                        </strong>
                      </li>
                      <li>
                        <strong>
                          <span style={{ "font-size": "14px" }}>
                            Sports injuries
                          </span>
                        </strong>
                      </li>
                      <li>
                        <span style={{ "font-size": "14px" }}>
                          <strong>
                            Brain injuries (like strokes) and other neurological
                            problems
                          </strong>
                        </span>
                      </li>
                    </ul>
                  </div>
                </div>
                <div
                  id="section-recruit"
                  className="detail-conten recruiterscrs clearfix coursedessside"
                >
                  <h2 className="heading-new">
                    <span>Top Recruiters</span>
                  </h2>{" "}
                  <div class="col-md-3">
                    <Link
                      to="http://www.hosmathospitals.in/"
                      target="_blank"
                      class="clearfix"
                    >
                      <img
                        src="https://www.careerpotli.com/afterrecruitmentimage/hosmat-hospital.jpg"
                        alt=""
                      />
                      <p>Hosmat Hospital</p>
                    </Link>
                  </div>
                  <div class="col-md-3">
                    <Link
                      to="https://www.helpageindia.org/"
                      target="_blank"
                      class="clearfix"
                    >
                      <img
                        src="https://www.careerpotli.com/afterrecruitmentimage/help-age-india.jpg"
                        alt=""
                      />
                      <p>Help Age India</p>
                    </Link>
                  </div>
                  <div class="col-md-3">
                    <Link
                      to="https://www.medicare.gov/"
                      target="_blank"
                      class="clearfix"
                    >
                      <img
                        src="https://www.careerpotli.com/afterrecruitmentimage/medicare.jpg"
                        alt=""
                      />
                      <p>Medicare</p>
                    </Link>
                  </div>
                  <div class="col-md-3">
                    <Link
                      to="https://www.manipalhospitals.com/"
                      target="_blank"
                      class="clearfix"
                    >
                      <img
                        src="https://www.careerpotli.com/afterrecruitmentimage/manipal-hospital.jpg"
                        alt=""
                      />
                      <p>Manipal Hospital</p>
                    </Link>
                  </div>
                  <div class="col-md-3">
                    <Link
                      to="https://www.maxhealthcare.in/"
                      target="_blank"
                      class="clearfix"
                    >
                      <img
                        src="https://www.careerpotli.com/afterrecruitmentimage/max-healthcare.jpg"
                        alt=""
                      />
                      <p>Max Healthcare</p>
                    </Link>
                  </div>
                  <div class="col-md-3">
                    <Link
                      to="http://careers.globalhospitalsindia.com/"
                      target="_blank"
                      class="clearfix"
                    >
                      <img
                        src="https://www.careerpotli.com/afterrecruitmentimage/global-hospitals.jpg"
                        alt=""
                      />
                      <p>Global Hospitals</p>
                    </Link>
                  </div>
                  <div class="col-md-3">
                    <Link
                      to="http://www.medanta.org/"
                      target="_blank"
                      class="clearfix"
                    >
                      <img
                        src="https://www.careerpotli.com/afterrecruitmentimage/medanta-medicity.jpg"
                        alt=""
                      />
                      <p>Medanta Medicity</p>
                    </Link>
                  </div>
                  <div class="col-md-3">
                    <Link
                      to="http://www.fortishealthcare.com/"
                      target="_blank"
                      class="clearfix"
                    >
                      <img
                        src="https://www.careerpotli.com/afterrecruitmentimage/fortis-healthcare.jpg"
                        alt=""
                      />
                      <p>Fortis Healthcare</p>
                    </Link>
                  </div>
                  <div class="col-md-3">
                    <Link
                      to="https://www.apollohospitals.com/"
                      target="_blank"
                      class="clearfix"
                    >
                      <img
                        src="https://www.careerpotli.com/afterrecruitmentimage/apollo-hospitals-enterprises.jpg"
                        alt=""
                      />
                      <p>Apollo Hospitals Enterprises</p>
                    </Link>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    );
  }
}
export default BOPhysiotherapy;
